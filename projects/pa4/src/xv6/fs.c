...
static int mkSmallFileDir(struct inode *ip, char *src, uint off, uint n) {
    ...
    if (ip->type == T_SMALL_FILE) {
        ...
        if (ip->size > SMALLFILE_SIZE) {
            return -1;
        }
        int i;
        int size = 0;
        char *current_address;
        for (i = off; i < off + n; i++, size++) {
            current_address = ((char *) ip->addrs) + i;
            *current_address = *(src + i - off);
        }
        size += off;
        ip->size = (size >= ip->size) ? size : (ip->size + n);
        iupdate(ip);
    } else {
        for (tot = 0; tot < n; tot += m, off += m, src += m) {
            uint idx = bmap(ip, off / BSIZE);
            if (!idx && n <= SMALLFILE_SIZE) {
                ip->size = SMALLFILE_SIZE;
                ip->type = T_SMALL_FILE;
                off = 0;
...
static int readSmallFileDir(struct inode *ip, char *dst, uint off, uint n) {
    ...
    //the data is stored directly in ip->addrs, copy that into dst
    int i;
    char *current_address;
    for (i = off; i < off + n; i++) {
        current_address = ((char *) ip->addrs) + i;
        *(dst + i - off) = *current_address;
    }
    return n;
}
...
readi(struct inode *ip, char *dst, uint off, uint n) {
    ...
    if (ip->type == T_SMALL_FILE) {
        return readSmallFileDir(ip, dst, off, n);
    } else {
...
writei(struct inode *ip, char *src, uint off, uint n) {
    ...
    return mkSmallFileDir(ip, src, off, n);
}
...
dirlookup(struct inode *dp, char *name, uint *poff) {
    ...
    for (off = 0; off < dp->size; off += BSIZE) {
        ...
        for (de = (struct dirent *) bp->data;
             ...
             de++) {
//      if(readi(dp, (char*)&de, off, BSIZE) != BSIZE)
//          panic("dirlink read");
            if (de->inum == 0)
                continue;
