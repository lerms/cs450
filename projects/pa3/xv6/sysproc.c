...
static int is_valid_pde(uint *flags) {
    return (PTE_P | PTE_W | PTE_U) & *flags;
}

static void check_flags(uint *present, uint *writable, uint *user, uint *ptef) {
...
//iterate over page table entries, check each of their flags.
